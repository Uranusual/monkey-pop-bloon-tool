...
gamemethod = os.path.join(BASE_DIR, "自定义打法.ini")
...
conf.read(gamemethod, encoding="utf-8")
    ...
    while True:
        ...
        try:
            ...
            if location_fail or location_continue:
                print('[%s]检测到失败，失败截图在image/fail中查看' % get_datetime())
...
def mupgrade(shortcut,mposition,combo=False, validation=True,show_log=False):
...
def fz_(shortcut,mposition, validation=True,slow=False):#目前有点小bug ,上面的猴子从,如果猴子放的时候钱不够且点到其他猴子,会导致判断为放置成功[617,45]附近区域会触发bug,解决方法,time.sleep使得有钱的时候再放
    if slow:
        time.sleep(1)
...
def tab(shortcut,mpsoiton,n):## 切换攻击模式
    print("[%s]切换%s的攻击对象%s次"%(get_datetime(),var_name(shortcut),n))
    click(global_point_map['game_right'],cd=0.1)
    click(mpsoiton)
    time.sleep(0.5)
    key(Key.tab, times=n)
    click(global_point_map['game_rightdown'], cd=0.1)


def aim_(shortcut,mpsoiton,aimposition):## 切换攻击模式
...
def aimtool(shortcut,mpsoiton,aimposition):## 切换位置
    ...
    for i in range(11):
        ...
        try:
            ...
            click((x,y),no_origin=[95,0],no_return=True)
            break
        except Exception as e:
            pass
    click(aimposition)
...
def aimcamo(shortcut,mpsoiton):## 优先攻击迷彩
...
# def dm_(x):#这个是根据像素来匹配的，不适用与位置不在中间的
#     print('[%s]移除位于%s的障碍物' % (get_datetime(),x))
#     click(global_point_map['game_rightdown'], times=2)
#     click(x)
#     time.sleep(1)
#     while True:
#         # if match_pixel(global_point_map['demolish'],(113, 232, 0)):
#         if match_pixel(global_point_map['demolish'],(85, 158, 105))!=True:
...
def abilities(x,n=1,cd=4,cd2=1):#循环使用技能线程失效
...
def ability(x,position=global_point_map['game_right'],cd=0):
    time.sleep(cd)
    key(x)
...
def msale(shortcut,x):
    print('[%s]出售位于%s的%s' % (get_datetime(),x,var_name(shortcut)))
    click(global_point_map['game_rightdown'], times=2)
    click(x) 
    key(Key.backspace)
...
def up001(shortcut,mposition):
    mupgrade(shortcut,mposition,'001')
def up002(shortcut,mposition):
    mupgrade(shortcut,mposition,'002')
def up003(shortcut,mposition):
    mupgrade(shortcut,mposition,'003')
def up004(shortcut,mposition):
    mupgrade(shortcut,mposition,'004')
def up005(shortcut,mposition):
    mupgrade(shortcut,mposition,'005')
# 第一个0
def up010(shortcut,mposition):
    mupgrade(shortcut,mposition,'010')
def up011(shortcut,mposition):
    mupgrade(shortcut,mposition,'011')
def up012(shortcut,mposition):
    mupgrade(shortcut,mposition,'012')
def up013(shortcut,mposition):
    mupgrade(shortcut,mposition,'013')
def up014(shortcut,mposition):
    mupgrade(shortcut,mposition,'014')
def up015(shortcut,mposition):
    mupgrade(shortcut,mposition,'015')

def up020(shortcut,mposition):
    mupgrade(shortcut,mposition,'020')
def up021(shortcut,mposition):
    mupgrade(shortcut,mposition,'021')
def up022(shortcut,mposition):
    mupgrade(shortcut,mposition,'022')
def up023(shortcut,mposition):
    mupgrade(shortcut,mposition,'023')
def up024(shortcut,mposition):
    mupgrade(shortcut,mposition,'024')
def up025(shortcut,mposition):
    mupgrade(shortcut,mposition,'025')
def up030(shortcut,mposition):

    mupgrade(shortcut,mposition,'030')
def up031(shortcut,mposition):
    mupgrade(shortcut,mposition,'031')
def up032(shortcut,mposition):
    mupgrade(shortcut,mposition,'032')
def up040(shortcut,mposition):
    mupgrade(shortcut,mposition,'040')
def up041(shortcut,mposition):
    mupgrade(shortcut,mposition,'041')
def up042(shortcut,mposition):
    mupgrade(shortcut,mposition,'042')
def up050(shortcut,mposition):
    mupgrade(shortcut,mposition,'050')
def up051(shortcut,mposition):
    mupgrade(shortcut,mposition,'051')
def up052(shortcut,mposition):
    mupgrade(shortcut,mposition,'052')

# 第2个0
def up101(shortcut,mposition):
    mupgrade(shortcut,mposition,'101')
def up110(shortcut,mposition):
    mupgrade(shortcut,mposition,'110')
def up120(shortcut,mposition):
    mupgrade(shortcut,mposition,'120')
def up102(shortcut,mposition):
    mupgrade(shortcut,mposition,'102')
def up103(shortcut,mposition):
    mupgrade(shortcut,mposition,'103')
def up104(shortcut,mposition):
    mupgrade(shortcut,mposition,'104')
def up105(shortcut,mposition):
    mupgrade(shortcut,mposition,'105')
def up200(shortcut,mposition):
    mupgrade(shortcut,mposition,'200')
def up201(shortcut,mposition):
    mupgrade(shortcut,mposition,'201')
def up202(shortcut,mposition):
    mupgrade(shortcut,mposition,'202')
def up203(shortcut,mposition):
    mupgrade(shortcut,mposition,'203')
def up204(shortcut,mposition):
    mupgrade(shortcut,mposition,'204')
def up205(shortcut,mposition):
    mupgrade(shortcut,mposition,'205')
    
def up301(shortcut,mposition):
    mupgrade(shortcut,mposition,'301')
def up302(shortcut,mposition):
    mupgrade(shortcut,mposition,'302')
def up401(shortcut,mposition):
    mupgrade(shortcut,mposition,'401')
def up402(shortcut,mposition):
    mupgrade(shortcut,mposition,'402')
def up501(shortcut,mposition):
    mupgrade(shortcut,mposition,'501')
def up502(shortcut,mposition):
    mupgrade(shortcut,mposition,'502')

def up101(shortcut,mposition):
    mupgrade(shortcut,mposition,'101')
def up100(shortcut,mposition):
    mupgrade(shortcut,mposition,'100')
def up220(shortcut,mposition):
    mupgrade(shortcut,mposition,'220')
def up420(shortcut,mposition):
    mupgrade(shortcut,mposition,'420')
def up520(shortcut,mposition):
    mupgrade(shortcut,mposition,'520')
def up230(shortcut,mposition):
    mupgrade(shortcut,mposition,'230')
def up240(shortcut,mposition):
    mupgrade(shortcut,mposition,'240')
def up250(shortcut,mposition):
    mupgrade(shortcut,mposition,'250')
def up320(shortcut,mposition):
    mupgrade(shortcut,mposition,'320')
def up510(shortcut,mposition):
    mupgrade(shortcut,mposition,'510')
def up500(shortcut,mposition):
    mupgrade(shortcut,mposition,'500')
def up150(shortcut,mposition):
    mupgrade(shortcut,mposition,'150')
def up300(shortcut,mposition):
    mupgrade(shortcut,mposition,'300')
def up400(shortcut,mposition):
    mupgrade(shortcut,mposition,'400')
def up410(shortcut,mposition):
    mupgrade(shortcut,mposition,'410')
def up130(shortcut,mposition):
    mupgrade(shortcut,mposition,'130')
def up140(shortcut,mposition):
    mupgrade(shortcut,mposition,'140')
def up210(shortcut,mposition):
    mupgrade(shortcut,mposition,'210')
...
def collect_chain():
    ...
    for i in range(1000):
        ...
        try:
            ...
            if need_save:
                print("[%s]恭喜你获得一个5阶insta，可以在image/insta中查看"%get_datetime())
