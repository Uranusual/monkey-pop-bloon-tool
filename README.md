...
- 使用源码运行
  ...
  1. 手动安装python，并安装所需的依赖包
  2. 到python[官网下载安装python](https://www.python.org/downloads/windows/) （推荐用[python3.8.10版本](https://www.python.org/ftp/python/3.8.10/python-3.8.10-amd64.exe),支持win7版本功能最全的版本，对win7和后续所有的windows兼容性最好）
  3. 安装所需依赖

  ```bash
  pip install pywin32 opencv-python==4.4.0.46  PyAutoGUI pynput #注意这里opencv的版本不能太高，不然路径不存在图片会报错以致刷屏
  ```
  安装完需要的环境就可以直接在文件夹中运行主程序启动了,也可以直接点击main.py文件直接运行
  ```
  cd code #进入code目录
